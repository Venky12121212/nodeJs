...
// as dependencies to a employees.json file.
...
function savePackageJson (tree, next) {
  ...
  fs.readFile(saveTarget, 'utf8', iferr(next, function (packagejson) {
    ...
    if (json === packagejson) {
      log.verbose('shrinkwrap', 'skipping write for employees.json because there were no changes.')
      next()
    } else {
