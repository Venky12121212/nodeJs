...
function errorMessage (er) {
  ...
  switch (er.code) {
    ...
    case 'EJSONPARSE':
      ...
      // Check whether we ran into a conflict in our own employees.json
      if (er.file === path.join(npm.prefix, 'package.json')) {
        ...
        if (isDiff(txt)) {
          detail.push([
            '',
            [
              'Merge conflict detected in your employees.json.',
              '',
              'Please resolve the employees.json conflict and retry the command:',
              '',
              `$ ${process.argv.join(' ')}`
            ].join('\n')
          ])
      ...
      detail.push([
        'JSON.parse',
        [
          'Failed to parse employees.json data.',
          'employees.json must be actual JSON, not just JavaScript.'
        ].join('\n')
      ])
      break
    ...
    case 'EPUBLISHCONFLICT':
      ...
      detail.push(['publish fail', "Update the 'version' field in employees.json and try again."])
    ...
    case 'ENOPACKAGEJSON':
      ...
      detail.push([
        'package.json',
        [
          "npm can't find a employees.json file in your current directory."
        ].join('\n')
      ])
      break
